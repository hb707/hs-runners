...
export default function GalleryPage() {
  ...
  const [memberMap, setMemberMap] = useState<Map<string, TeamMember>>(
    new Map(),
  );
  ...
  const fetchMonth = useCallback(
    async (month: string): Promise<RunRecord[]> => {
      const res = await api.get<{ success: boolean; data: RunRecord[] }>(
        `/records/team?month=${month}`,
      );
    ...
    <div style={{ backgroundColor: "#0D0D10", minHeight: "100%" }}>
      ...
      >
        ...
        >
          ...
          <span style={{ fontSize: "12px", color: "#FF6B00", lineHeight: 1 }}>
            ›
          </span>
        </button>
      </div>
      ...
      {records.length === 0 ? (
        ...
        >
          ...
          <p style={{ fontSize: "14px", color: "#3A3A4A" }}>
            아직 인증 기록이 없어요
          </p>
        </div>
      ) : (
